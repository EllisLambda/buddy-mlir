...
public:
  ...
  matchAndRewrite(Operation *op, ArrayRef<Value> /*operands*/,
    ...
    const AffineExpr dim0 = rewriter.getAffineDimExpr(0);
    const AffineExpr dim1 = rewriter.getAffineDimExpr(1);
    const AffineExpr sym0 = rewriter.getAffineSymbolExpr(0);
    ...
    Value tailLength = rewriter.create<affine::AffineApplyOp>(
        loc, AffineMap::get(1, 0, dim0 % vecSize), ValueRange{bCol});
    ...
    Value appliedColOfB = rewriter.create<affine::AffineApplyOp>(
        loc, AffineMap::get(1, 0, dim0.ceilDiv(vecSize)), ValueRange{bCol});
    ...
    rewriter.create<affine::AffinePrefetchOp>(
        loc, A, AffineMap::get(2, 0, {dim0, dim1}, rewriter.getContext()),
    ...
    affine::AffineIfOp branchingOp = rewriter.create<affine::AffineIfOp>(
        loc, IntegerSet::get(1, 1, {dim0 * -vecSize + sym0 - vecSize}, {false}),
    ...
    affine::buildAffineLoopNest(
        ...
        [&](OpBuilder &builder, Location loc, ValueRange ivRange) {
          ...
          Value bVec = builder.create<affine::AffineVectorLoadOp>(
              loc, VectorType::get({vecSize}, elementType), B,
              AffineMap::get(2, 0, {dim0, dim1 * vecSize},
                             rewriter.getContext()),
    ...
    affine::buildAffineLoopNest(
        ...
        [&](OpBuilder &builder, Location loc, ValueRange ivRange) {
          ...
          Value tailIdxColOfB = builder.create<affine::AffineApplyOp>(
              loc, AffineMap::get(1, 0, dim0 * vecSize),
...
class MatMulParallelVectorizationPass
    : public PassWrapper<MatMulParallelVectorizationPass, OperationPass<ModuleOp>> {
public:
  MLIR_DEFINE_EXPLICIT_INTERNAL_INLINE_TYPE_ID(MatMulParallelVectorizationPass)
  StringRef getArgument() const final { return "matmul-paralell-vectorization-optimize"; }
